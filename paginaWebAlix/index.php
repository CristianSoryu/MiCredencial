...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // ✅ FIX 1: faltaba WHERE y el ? del parámetro
        $sql  = "SELECT id_usuario, contrasena, nombres FROM usuarios WHERE id_usuario = ?";
        $stmt = $conexion->prepare($sql);

        // ✅ FIX 2: bindParam con ? usa índice 1, no nombre
        ...
        if ($user && ($password == $user['contrasena'] || empty($user['contrasena']))) {
            ...
            $_SESSION['usuario_documento'] = $documento; // ✅ FIX 3: no existe columna 'documento', usamos la variable
