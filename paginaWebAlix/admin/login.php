...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // 1. Ahora atrapamos 'username' tal cual como lo envía el formulario HTML
    $username = $_POST['username']; 
    $password = $_POST['password'];

    try {
        // 2. Buscamos en la columna user_handle (o en email, por si acaso)
...
<head>
    ...
    <title>Acceso Admin - Universidad Libre</title>
...
<body>
    <header class="header">
        ...
        <h2 style="color: white; margin-top: 20px; text-align: center;">Acceso administrativo</h2>
    </header>

    <div class="contenedor">
        <div class="login">
            <p style="color: #666; font-size: 14px; line-height: 1.5;">
                Ingresa tu usuario y contraseña para acceder al panel de administracion.
            </p>

            <form id="admin-login-form" action="" method="POST">
    <!-- Aquí aparecerá el error si la contraseña está mal -->
    <?php if(!empty($error)): ?>
        <p style="color: red; text-align: center; font-weight: bold;"><?php echo $error; ?></p>
    <?php endif; ?>

    <label for="admin-username">Usuario</label>
    <!-- Agregamos name="username" -->
    <input type="text" id="admin-username" name="username" placeholder="Ingresa tu usuario" required>

    <label for="admin-password">Contraseña</label>
    <!-- Agregamos name="password" -->
    <input type="password" id="admin-password" name="password" placeholder="Ingresa tu contraseña" required>
